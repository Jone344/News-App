...
extension SharedVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = newsTableView.dequeueReusableCell(withIdentifier: "newsCellShared", for: indexPath) as? NewsCellShared else { return UITableViewCell() }
...
extension SharedVC {
    ...
    func getNewsData(complete: @escaping (_ status: Bool) -> ()) {
        ...
        Alamofire.request("https://api.nytimes.com/svc/mostpopular/v2/shared/30.json?api-key=0l1QWFx2rTrt0PjzcZYohV9TdqiPiQyz", method: .get).responseJSON { [self] (response) in
